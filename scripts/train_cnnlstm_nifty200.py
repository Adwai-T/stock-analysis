...
MODEL_PATH = MODEL_DIR / "cnn_lstm_nifty200.h5"          # <-- new name
SCALER_PATH = MODEL_DIR / "cnn_lstm_nifty200_scaler.npz" # <-- new name
...
def train_model():
    ...
    model = Sequential([
        # Conv over time dimension (seq_len), channels = features
    ...
    print("Starting training (CNN-LSTM)...")
    history = model.fit(
        X_train, y_train,
