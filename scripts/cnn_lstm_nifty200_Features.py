...
MODEL_PATH = MODEL_DIR / "cnn_lstm_nifty200_features.h5"
SCALER_PATH = MODEL_DIR / "cnn_lstm_nifty200_features_scaler.npz"
...
def build_dataset():
    """
    Build X, y from all NIFTY 200 stocks present in data/allData using engineered features.
...
def build_cnn_lstm_model(seq_len: int, num_features: int):
    """
    Build a hybrid CNN-LSTM model:
    Conv1D -> Conv1D -> LSTM -> Dense -> Output
    """
    model = Sequential([
        Conv1D(
            filters=32,
...
def train_model():
    ...
    model = build_cnn_lstm_model(SEQ_LEN, num_features)
    ...
    print("Starting training (CNN-LSTM NIFTY200)...")
    history = model.fit(
        X_train, y_train,
