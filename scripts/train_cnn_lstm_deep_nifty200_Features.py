...
MODEL_PATH = MODEL_DIR / "cnn_lstm_deep_nifty200.h5"
SCALER_PATH = MODEL_DIR / "cnn_lstm_deep_nifty200_scaler.npz"
...
def build_cnn_lstm_deep(seq_len: int, num_features: int):
    """
    Deeper CNN-LSTM:
    Conv1D -> Conv1D -> LSTM -> LSTM -> Dense
    """
    model = Sequential([
        Conv1D(
            filters=64,
...
def train_model():
    ...
    model = build_cnn_lstm_deep(SEQ_LEN, num_features)
    ...
    print("Starting training (Deep CNN-LSTM NIFTY200)...")
    history = model.fit(
        X_train, y_train,
