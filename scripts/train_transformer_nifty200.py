...
MODEL_PATH = MODEL_DIR / "transformer_nifty200.h5"
SCALER_PATH = MODEL_DIR / "transformer_nifty200_scaler.npz"
...
def build_transformer_model(seq_len: int, num_features: int) -> Model:
    ...
    # Linear projection from features -> model dimension
    x = Dense(d_model)(inputs)

    # Positional embedding
    positions = tf.range(start=0, limit=seq_len, delta=1)
    pos_embedding_layer = Embedding(input_dim=seq_len, output_dim=d_model)
    pos_encoding = pos_embedding_layer(positions)          # (seq_len, d_model)
    ...
    ffn = Dense(d_ff, activation="relu")(out1)
